...
export function createGlassCompositor(renderer,cinema){
 ...
 const blurred=new THREE.WebGLRenderTarget(1,1,{type:THREE.HalfFloatType,depthBuffer:false}),scratch=blurred.clone();
 const blurMaterial=new THREE.ShaderMaterial({depthTest:false,depthWrite:false,toneMapped:false,uniforms:{source:{value:target.texture},direction:{value:new THREE.Vector2()}},vertexShader:'varying vec2 screenUv;void main(){screenUv=uv;gl_Position=vec4(position.xy,0.,1.);}',fragmentShader:`varying vec2 screenUv;uniform sampler2D source;uniform vec2 direction;void main(){vec3 sum=vec3(0.);float weights=0.;for(int i=-12;i<=12;i++){float f=float(i),weight=exp(-f*f/32.);sum+=texture2D(source,clamp(screenUv+direction*f,vec2(.001),vec2(.999))).rgb*weight;weights+=weight;}gl_FragColor=vec4(sum/weights,1.);}`});
 const material=new THREE.ShaderMaterial({depthTest:false,depthWrite:false,uniforms:{source:{value:target.texture},blurred:{value:blurred.texture},resolution:{value:new THREE.Vector2(1,1)},pointer:{value:new THREE.Vector2(-1000,-1000)},cinema,rects:{value:rects},params:{value:params},count:{value:0}},
 vertexShader:'varying vec2 screenUv;void main(){screenUv=uv;gl_Position=vec4(position.xy,0.,1.);}',
 fragmentShader:`varying vec2 screenUv;uniform sampler2D source;uniform sampler2D blurred;uniform vec2 resolution;uniform vec2 pointer;uniform float cinema;uniform vec4 rects[24];uniform vec4 params[24];uniform int count;
 ...
 for(int i=0;i<24;i++){
  ...
  vec3 transmitted=optics.w<.5?texture2D(blurred,screenUv).rgb:diffuseScene(pixel+bend,optics.y);
  ...
  float edge=exp(-interior/1.05)*step(.5,optics.w);vec2 lightDirection=normalize(vec2(-.65,.9));
  ...
  glass+=vec3(.09,.11,.14)*exp(-interior/4.)*bevelLight*step(.5,optics.w);
 ...
 const blurScene=new THREE.Scene(),blurQuad=new THREE.Mesh(new THREE.PlaneGeometry(2,2),blurMaterial);blurScene.add(blurQuad);
 let viewportHeight=1,viewportWidth=1;
 function measure(){
 ...
 return {target,resize(pixelWidth,pixelHeight,width,height){target.setSize(pixelWidth,pixelHeight);blurred.setSize(Math.max(1,Math.ceil(pixelWidth/4)),Math.max(1,Math.ceil(pixelHeight/4)));scratch.setSize(blurred.width,blurred.height);viewportHeight=height;viewportWidth=width;material.uniforms.resolution.value.set(width,height);measure();},measure,render(){
 blurMaterial.uniforms.source.value=target.texture;blurMaterial.uniforms.direction.value.set(3.5/viewportWidth,0);renderer.setRenderTarget(scratch);renderer.render(blurScene,camera);
 blurMaterial.uniforms.source.value=scratch.texture;blurMaterial.uniforms.direction.value.set(0,3.5/viewportHeight);renderer.setRenderTarget(blurred);renderer.render(blurScene,camera);
