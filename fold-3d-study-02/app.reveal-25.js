...
function cameraIsland(){
 ...
 for(let j=0;j<=layers;j++){
  ...
  for(let i=0;i<count;i++)positions.push(contour[i].x,contour[i].y,-.145-.140*t);
 }
...
const frag=`precision highp float;
 ...
 vec2 imageRevealField(vec2 imagePoint,float p,bool cover){
  float u=clamp(cover?1.-imagePoint.x/3.14:-imagePoint.x/3.14,0.,1.);
  float rise=smoothstep(0.,.5,p);
